...
(int strings_count,
 ...
 int* result_count) 
 {
    // the result count be equal to number of queries
    *result_count = queries_count;

    // Create a result array to store answers
    int* result = malloc(queries_count * sizeof(int));

    // loop each queries given
    for (int i = 0; i < queries_count; i++) {
        int count = 0;

        // For each query, check all strings
        for (int j = 0; j < strings_count; j++) {
            // If string matches ty increse
            if (strcmp(queries[i], strings[j]) == 0) {
                count++;  // Increase count
            }
        }

        // Store the final count for this query
        result[i] = count;
    }
