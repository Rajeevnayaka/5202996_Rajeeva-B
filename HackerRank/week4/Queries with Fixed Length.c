...
int* solve(int arr_count, int* arr, int queries_count, int* queries, int* result_count) {
    *result_count = queries_count;
    ...
    for (int qi = 0; qi < queries_count; qi++) {
        int k = queries[qi];  // current subarray size

        // Create deque
        Deque dq;
        dq.data = malloc(arr_count * sizeof(int));
        dq.front = 0;
        dq.back = -1;
        ...
        for (int i = 0; i < arr_count; i++) {
            ...
            while(dq.back >= dq.front && arr[i] >= arr[dq.data[dq.back]]) dq.back--;
            dq.data[++dq.back] = i;

            // remove elements outside the window
            if(dq.data[dq.front] <= i - k) dq.front++;

            if(i >= k - 1) {  // window fully formed
                if(arr[dq.data[dq.front]] < min_of_max)
                    min_of_max = arr[dq.data[dq.front]];
            }
        }
