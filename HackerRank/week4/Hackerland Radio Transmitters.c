...
int hackerlandRadioTransmitters(int x_count, int* x, int k) {
    ...
    while (i < x_count) {
        transmitters++;  // we will place a transmitter

        // Find the farthest house to the right we can cover starting from x[i]
        int loc = x[i] + k;
        while (i < x_count && x[i] <= loc) {
            i++;
        }

        // Place transmitter at the last house we can reach
