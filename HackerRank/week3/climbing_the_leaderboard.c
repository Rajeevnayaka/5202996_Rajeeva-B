...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    ...
    *result_count = player_count;
    ...
    // Step 1: Remove duplicates from ranked and prepare unique leaderboard
    int *unique = malloc(ranked_count * sizeof(int));
    int uniq_size = 0;
    unique[uniq_size++] = ranked[0];
    for (int i = 1; i < ranked_count; i++) {
        if (ranked[i] != unique[uniq_size - 1]) {
            unique[uniq_size++] = ranked[i];
        }
    }

    // Step 2: For each player score, determine rank (from bottom up for efficiency)
    int idx = uniq_size - 1;  // start from lowest rank
    for (int i = 0; i < player_count; i++) {
        int score = player[i];
        while (idx >= 0 && score >= unique[idx]) {
            idx--;  // move up leaderboard
        }
        // If idx < 0 => player is higher than all
        res[i] = idx + 2; 
        // (idx + 1) would be current rank, +1 more because 0-based index
    }
