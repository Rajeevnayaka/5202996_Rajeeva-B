 ...
 *
 */
// Helper function to generate first q prime numbers
...
int* waiter(int number_count, int* number, int q, int* result_count) {
    ...
    int* curr_stack = malloc(number_count * sizeof(int));
    for (int i = 0; i < number_count; i++)
        curr_stack[i] = number[i];
    int top = number_count - 1;

    for (int i = 0; i < q; i++) {
        int prime = primes[i];

        int* b_stack = malloc(number_count * sizeof(int));
        int b_top = -1;
        ...
        for (int j = top; j >= 0; j--) {
            int val = curr_stack[j];
            if (val % prime == 0)
                b_stack[++b_top] = val;
            else
                next_stack[++next_top] = val;
        }

        // Push B stack to result
        while (b_top >= 0)
            result[res_idx++] = b_stack[b_top--];

        // Prepare for next iteration
        for (int j = 0; j <= next_top; j++)
            curr_stack[j] = next_stack[j];
        top = next_top;
    ...
    // Push remaining A stack to result
    while (top >= 0)
        result[res_idx++] = curr_stack[top--];
