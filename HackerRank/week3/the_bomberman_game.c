 ...
 *
 */
static char** make_full(int r, int c, int* result_count) {
    *result_count = r;
...
static char** copy_grid(char** grid, int r, int c, int* result_count) {
    *result_count = r;
    char** out = (char**)malloc(r * sizeof(char*));
    for (int i = 0; i < r; i++) {
        out[i] = (char*)malloc((c + 1) * sizeof(char));
...
static char** explode(char** src, int r, int c, int* result_count) {
    *result_count = r;
    char** out = (char**)malloc(r * sizeof(char*));
    for (int i = 0; i < r; i++) {
        out[i] = (char*)malloc((c + 1) * sizeof(char));
...
char** bomberMan(int n, int r, char** grid, int* result_count) {
    ...
    if (n == 1) {
        return copy_grid(grid, r, c, result_count);
    }

    if (n % 2 == 0) {
        return make_full(r, c, result_count);
    }

    int tmp_count;
    char** after3 = explode(grid, r, c, &tmp_count);
    if (n % 4 == 3) {
        *result_count = r;
        return after3;
    }

    char** after5 = explode(after3, r, c, &tmp_count);
    *result_count = r;
    return after5;
}
